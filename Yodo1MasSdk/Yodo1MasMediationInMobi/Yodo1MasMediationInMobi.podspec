#
# Be sure to run `pod lib lint Yodo1AdvertAdMob.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'Yodo1MasMediationInMobi'
  s.version          = '1.0.0'
  s.summary          = 'Yodo1MasMediationInMobi'
  s.swift_version    = '5.0'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
TODO: Add long description of the pod here.
                       DESC

  s.homepage         = 'https://github.com/Yodo1/Yodo1MasMediationInMobi'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'ZhouYuzhen' => 'zhouyuzhen@yodo1.com' }
  s.source           = { :git => 'https://github.com/Yodo1/Yodo1MasMediationInMobi.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '9.0'
  # s.static_framework = true

  s.source_files = 'Yodo1MasMediationInMobi/Classes/**/*'
  
  s.resource_bundles = {
    'Yodo1MasMediationInMobi' => ['Yodo1MasMediationInMobi/Assets/*']
  }
  # s.resources = 'Yodo1MasMediationInMobi/Assets/*.strings', 'Yodo1MasMediationInMobi/Assets/*.xcassets'

  # s.public_header_files = 'Pod/Classes/**/*.h's
  # s.frameworks = 'UIKit'
  s.dependency 'Yodo1MasCore'
  s.dependency 'InMobiSDK'#, '9.1.1'
  
end
